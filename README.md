...
- 嵌套路径自动同步： 支持类似 'users[0].profile.name' 的路径字符串，自动解析数组索引与嵌套对象层级，精准同步深层属性。
- 双向同步（可选）： 通过 { bidirectional: true } 开启，目标 ref 改变时会自动回写源对象，实现真正的数据双向联动。
...
],{bidirectional: true}) // bidirectional开启双向同步(当目标对象被修改时，自动更新源对象)
